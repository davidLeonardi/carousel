...
dojo.declare("dojox.image.Carousel", [dijit._Widget, dijit._Templated], {
	...
	getSets: function(){
		...
		var items = [];
		dojo.forEach(this._internalDataStore.query(), function(item){items.push(item.setName)});
		items.sort();
		var j = 0;
	...
	getImageDataItemByIndex: function(index){
		...
		return this._internalDataStore.query(dojo.mixin({index: index}, this._imageSetStoreQuery))[0];
	},
	...
	applyImageSet: function(oArgs){
		...
		if((this._imageSetStoreQuery)&&(this._imageSetStoreQuery.setName == oArgs.setName)){return}

		oArgs.setName = oArgs.setName || this.defaultSet;
		
		this._imageSetStoreQuery = {setName: oArgs.setName};
	...
	showIndex: function(index){
		...
		if(this._currentImageDataItem){var currentImageNode = dojo.byId(this._currentImageDataItem.imageNodeId);}


		//don't do anything if we're requesting the same image as currently displayed
		if (this._currentImageDataItem && (this._currentImageDataItem === this._nextImageDataItem)){return;}
		...
		if(currentImageNode){	
			dojo.style(nextImageNode, {display: "block", position: "absolute", left: 0, top: 0});
			dojo.style(currentImageNode, {opacity: 0, display: "none"});
			dojo.style(nextImageNode,{opacity: 1});
			} else {
			dojo.style(nextImageNode, {display: "block", position: "absolute", left: 0, top: 0, opacity: 1});	
		}
	...
	showNext: function(){
		...
		if(this._internalDataStore.query(this._imageSetStoreQuery).length < newIndex){return;}	
    ...
    _setupData: function() {
		...
		this.applyImageSet({setName: this.initialSet || this._defaultSet});
    ...
    _addDataFromNode: function(sourceNode) {
        ...
        dojo.query(this.childImageQuery, sourceNode).forEach(function(currentNode, index) {
            ...
            dojo.place(dojo.attr(currentNode, {"id": currentID, style: {"opacity": "0", "display": "none"}}), this.nodeCache, "last");
			this._addNewItemToStore(this._createDataItemFromNode({currentID: currentID, currentNode: currentNode, metaDataNodes: metaDataNodes, setName:setName}));
