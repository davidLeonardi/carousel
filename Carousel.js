...
//NOTE: videos MUST be have preload="none" to disable autoloading!!!!

//special "autoplay" when shown feauture: add data-carousel-play-when-shown="true" to the <video>'s node in the original markup
//special "loop" feature: add data-carousel-loop-video="true" to the video node
    ...
    getSourceForNode: function(node) {
        ...
        if (node.tagName === "IMG") {
            if(node.src){
                ...
                alert("no source for image has been specified in the markup");
            }
        } else if (node.tagName === "VIDEO" || node.tagName === "video") {
            var sources = [];
